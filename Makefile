# $MCom: portstools/tinderbox/Makefile,v 1.30.2.40 2008/06/02 03:49:38 marcus Exp $

VERSION=	2.4.2

DATA=		README buildscript create ds.ph.dist makemake \
		mkbuild mkjail pnohang.c portbuild rawenv.dist tbkill.sh tc \
		tinderbox.ph.dist tinderbox-mysql.schema  \
		tinderbox-pgsql.schema tinderbuild tinderd upgrade.sh \
		setup.sh enterbuild
LIBDATA=	Build.pm BuildPortsQueue.pm Hook.pm Host.pm Jail.pm \
		MakeCache.pm Port.pm PortsTree.pm TBConfig.pm TinderObject.pm \
		TinderboxDS.pm User.pm tinderlib.pl tinderbox_shlib.sh \
		setup_shlib.sh setup-mysql.sh setup-pgsql.sh enterbuild \
		PortFailPattern.pm PortFailReason.pm
...
MIGDATA=	mig_shlib.sh mig_mysql_tinderbox-1.X_to_2.0.0.sql \
		...
		mig_mysql_tinderbox-2.1.1_to_2.2.0.sql \
		mig_pgsql_tinderbox-2.1.1_to_2.2.0.sql \
		mig_mysql_tinderbox-2.2.0_to_2.3.0.sql \
		mig_pgsql_tinderbox-2.2.0_to_2.3.0.sql \
...
WWWDATA=	Build.php Jail.php Port.php PortsTree.php TinderObject.php \
		TinderboxDS.php inc_ds.php.dist failures.php index.php \
		lastbuilds.php showbuild.php showport.php \
		inc_tinderbox.php.dist tinderstyle.css Makefile
WWWEXPDATA=	inc_ds.php.dist inc_tinderbox.php.dist index.php
